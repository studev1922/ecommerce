...
const query = {
    ...
    /**
     * @param {String} query in transaction
     * @param {Boolean} isThrow is throw
     * @returns 
     */
    ...
    /**
     * 
     * @param {String} proceName 
     * @param {Array<Array>} data 
     * @returns {String} query execute once time or execute multiple times
     */
    procedure: (proceName, data) => Array.isArray(data)
        ? data.map(e => `EXECUTE ${proceName} ${modify(Object.values(e))}`).join(';\n')
        : `EXECUTE ${proceName} ${modify(Object.values(data))}`,
