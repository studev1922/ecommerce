...
CREATE PROCEDURE SP_REGISTER
    ...
    @roles varchar(max) -- UNIQUE role is name EX: 'ADMIN,USER,OWNER'
AS BEGIN
...
-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++ CREATE TRIGGERS
-- IF EXISTS (SELECT [object_id] FROM sys.triggers WHERE name = N'TG_USERS') DROP TRIGGER TG_USERS
-- GO
-- CREATE TRIGGER TG_USERS ON USERS
-- AFTER INSERT AS BEGIN -- return all products after inserted
-- 	DECLARE @qty int = (SELECT COUNT(uid) FROM inserted);
--     DECLARE @size int = (SELECT COUNT(uid) FROM USERS);
--     SELECT * FROM VIEW_USERS ORDER BY regTime 
...
AFTER INSERT AS BEGIN -- return all products after inserted
    ...
    SELECT * FROM VIEW_PRODUCTS ORDER BY prid 
    OFFSET (@size-@qty) ROWS FETCH NEXT @qty ROWS ONLY;
END
GO
...
EXECUTE SP_REGISTER 'owner', '123', 'Owner System', null, 'OWNER,ADMIN,STAFF,SELLER,USER';
EXECUTE SP_REGISTER 'admin', '123', 'Admin System', null, 'ADMIN,STAFF,USER';
EXECUTE SP_REGISTER 'staff', '123', 'Staff System', null, 'STAFF,USER';
EXECUTE SP_REGISTER 'seller', '123', 'Seller System', null, 'SELLER';
EXECUTE SP_REGISTER 'user', '123', 'User System', null, 'USER';
